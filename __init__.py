"""ComfyUI nodes for planning and writing MiniMax H3 prompts."""

if __package__:
    from .plan_adapter import (
        NODE_CLASS_MAPPINGS as PLAN_ADAPTER_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PLAN_ADAPTER_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from .prompt_review import (
        NODE_CLASS_MAPPINGS as PROMPT_REVIEW_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PROMPT_REVIEW_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from . import prompt_review_server as _prompt_review_server  # noqa: F401
    from .plan_enhancer import (
        NODE_CLASS_MAPPINGS as PLAN_ENHANCER_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PLAN_ENHANCER_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from .plan_v2 import (
        NODE_CLASS_MAPPINGS as PLAN_V2_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PLAN_V2_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from .enhancer import (
        NODE_CLASS_MAPPINGS as ENHANCER_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as ENHANCER_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from .nodes import (
        NODE_CLASS_MAPPINGS as GUIDE_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as GUIDE_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from .media_context import (
        NODE_CLASS_MAPPINGS as MEDIA_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as MEDIA_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from .reference_sheet import (
        NODE_CLASS_MAPPINGS as SHEET_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as SHEET_NODE_DISPLAY_NAME_MAPPINGS,
    )
else:  # Allows direct loading by test runners from a hyphenated folder.
    from plan_adapter import (
        NODE_CLASS_MAPPINGS as PLAN_ADAPTER_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PLAN_ADAPTER_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from prompt_review import (
        NODE_CLASS_MAPPINGS as PROMPT_REVIEW_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PROMPT_REVIEW_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from plan_enhancer import (
        NODE_CLASS_MAPPINGS as PLAN_ENHANCER_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PLAN_ENHANCER_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from plan_v2 import (
        NODE_CLASS_MAPPINGS as PLAN_V2_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as PLAN_V2_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from enhancer import (
        NODE_CLASS_MAPPINGS as ENHANCER_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as ENHANCER_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from nodes import (
        NODE_CLASS_MAPPINGS as GUIDE_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as GUIDE_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from media_context import (
        NODE_CLASS_MAPPINGS as MEDIA_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as MEDIA_NODE_DISPLAY_NAME_MAPPINGS,
    )
    from reference_sheet import (
        NODE_CLASS_MAPPINGS as SHEET_NODE_CLASS_MAPPINGS,
        NODE_DISPLAY_NAME_MAPPINGS as SHEET_NODE_DISPLAY_NAME_MAPPINGS,
    )

NODE_CLASS_MAPPINGS = {
    **PLAN_V2_NODE_CLASS_MAPPINGS,
    **PLAN_ENHANCER_NODE_CLASS_MAPPINGS,
    **PLAN_ADAPTER_NODE_CLASS_MAPPINGS,
    **PROMPT_REVIEW_NODE_CLASS_MAPPINGS,
    **GUIDE_NODE_CLASS_MAPPINGS,
    **MEDIA_NODE_CLASS_MAPPINGS,
    **SHEET_NODE_CLASS_MAPPINGS,
    **ENHANCER_NODE_CLASS_MAPPINGS,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    **PLAN_V2_NODE_DISPLAY_NAME_MAPPINGS,
    **PLAN_ENHANCER_NODE_DISPLAY_NAME_MAPPINGS,
    **PLAN_ADAPTER_NODE_DISPLAY_NAME_MAPPINGS,
    **PROMPT_REVIEW_NODE_DISPLAY_NAME_MAPPINGS,
    **GUIDE_NODE_DISPLAY_NAME_MAPPINGS,
    **MEDIA_NODE_DISPLAY_NAME_MAPPINGS,
    **SHEET_NODE_DISPLAY_NAME_MAPPINGS,
    **ENHANCER_NODE_DISPLAY_NAME_MAPPINGS,
}
WEB_DIRECTORY = "./web"

__all__ = ["NODE_CLASS_MAPPINGS", "NODE_DISPLAY_NAME_MAPPINGS", "WEB_DIRECTORY"]
