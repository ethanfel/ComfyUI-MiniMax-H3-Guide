...
def test_enhancer_tooltips_explain_outputs_and_optional_image_scope():
    ...
    assert schema["optional"]["offload_after_generation"][1]["default"] is False
    assert (
        "never explicitly unloads"
    ...
    assert schema["required"]["max_new_tokens"][1]["default"] == 1200
    assert "800-1200" in schema["required"]["max_new_tokens"][1]["tooltip"]
...
def test_complete_qwen_never_forces_unload_even_when_legacy_switch_is_on(
    monkeypatch,
):
    model_management = ModuleType("comfy.model_management")
    model_management.unload_model_and_clones = lambda *_args, **_kwargs: pytest.fail(
        "the enhancer must not synchronously unload a connected complete Qwen model"
    )
    comfy = ModuleType("comfy")
    comfy.__path__ = []
    comfy.model_management = model_management
...
def test_complete_qwen_generation_failure_never_forces_unload(monkeypatch):
    model_management = ModuleType("comfy.model_management")
    model_management.unload_model_and_clones = lambda *_args, **_kwargs: pytest.fail(
        "error cleanup must not synchronously unload a connected complete Qwen model"
    )
    comfy = ModuleType("comfy")
    comfy.__path__ = []
    comfy.model_management = model_management
